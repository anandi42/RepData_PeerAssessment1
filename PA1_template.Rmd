...
byint$timeint<-as.POSIXct(sprintf("%04d",byint_plot$interval), tz="UTC", format="%H%M")
```
...
```
  ...
  There are `r nrow(bydate_missing)` missing rows to impute out of a total `r nrow(activity)`, and these missing rows are associated with 8 days out of the 2-month dataset. We replace all the values of `steps` in `missing` with the corresponding mean value from `byint`. If we had a much larger dataset this solution may need to be changed fro something faster. 

...
We first create summary datasets of the total steps per day, and the daily activity pattern. The two generated dataframes are:  
  ...
  * `r "bydate"`: Summed steps by day
  * `r "byint"`: average steps at each 5-min interval, averaged over all days 
...
To answer the question, we plot a histogram of `r "bydate"`, which contains the summed steps taken for every day of the dataset. Using the  pacakge [gridExtra](http://cran.r-project.org/web/packages/gridExtra/gridExtra.pdf), we can annotate the plot with the calculated mean and median. 
...
The dataframe `r "byint"` contains the average steps at each 5-min interval. Each interval is averaged across all days reported in the data (currently with NAs removed). In the next figure, we plot the average steps against the time interval. The table reports the maximum steps and the associated interval. 
...
There are `r nrow(bydate_missing)` missing rows to impute out of a total `r nrow(activity)`, and these missing rows are associated with 8 days out of the 2-month dataset. 
...
```{r plot4, dependson=c(2,3,6), fig.cap="Daily activity on Weekdays vs. Weekends", fig.height=8}
...
```{r extra, dependson=c(2,3,6,7,8), fig.cap="Additional plots", fig.width=10, fig.height=8}
act5 <- act3 %>%
  group_by(date,dow) %>%
  summarize(steps=sum(steps))
